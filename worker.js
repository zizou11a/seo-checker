...
async function analyzeSite(targetUrl) {
  ...
  // Fetch robots.txt, sitemap & PageSpeed in parallel
  const [robotsResult, sitemapResult, pagespeedResult] = await Promise.allSettled([
...
function calcScore(checks) {
  ...
  const CATEGORIES = {
    ...
    social: {
      ...
      weight: 0.10,
      checks: {
        openGraph: { good: 60, warning: 30, error: 0 },
        lang:      { good: 40, warning: 20, error: 0 },
      }
    },
    structured: {
